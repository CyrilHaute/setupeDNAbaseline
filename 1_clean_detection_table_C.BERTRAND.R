...
# The data subset resulting from this script will be used in another scipt to combine occurence by transect
# Output of this script: data_MED_teleo_pres_1824_V1.csv
# ──────────────────────────────────────────────────────
...
raw <- readxl::read_xlsx(
  path = "data/raw_data/Results_Teleo_2018-2024_with_IPOCOM.xlsx",
  col_names = FALSE
)
...
header_row <- which(
  apply(raw, 1, function(x) {
    any(grepl("^scientific_name$", x, ignore.case = TRUE))
  })
)
if (length(header_row) == 0) {
  stop("Header row with 'scientific_name' not found.")
}
...
hdr[hdr == "" | is.na(hdr)] <- paste0(
  "unnamed_",
  seq_len(sum(hdr == "" | is.na(hdr)))
)
...
marmo_col <- names(data_all)[grepl(
  "^Dasyatis marmorata$",
  names(data_all),
  ignore.case = TRUE
)]
torto_col <- names(data_all)[grepl(
  "^Dasyatis tortonesei$",
  names(data_all),
  ignore.case = TRUE
)]
...
if (length(marmo_col) > 0 & length(torto_col) > 0) {
  message(
    "Merging detections of 'Dasyatis marmorata' into 'Dasyatis tortonesei'."
  )

  # Merge numeric values safely (handle NAs)
  data_all[[torto_col]] <- rowSums(
    data_all[, c(marmo_col, torto_col)],
    na.rm = TRUE
  )
  ...
  message(
    "Renaming 'Dasyatis marmorata' → 'Dasyatis tortonesei' (no existing D. tortonesei column)."
  )
...
# Load
metadatas <- read.csv(
  "data/raw_data/Med_metadonnees_ADNe - v1.2_2018-2024.csv",
  sep = ",",
  dec = "."
) |>
  dplyr::rename(sample = spygen_code) |>
  dplyr::filter(Tele01 == 1) |>
  dplyr::mutate(pool = gsub("\\.", "_", pool)) |>
  dplyr::mutate(
    pool = ifelse(grepl("no", pool, ignore.case = TRUE), sample, pool)
  ) |>

  # ── Ensure pooled IDs are sorted alphabetically ("croissant")
  dplyr::mutate(
    pool = sapply(pool, function(x) {
...
check_pools <- metadatas |>
  dplyr::filter(grepl("SPY", pool) & grepl("_", pool)) |>
  dplyr::mutate(
    is_sorted = sapply(pool, function(x) {
      parts <- unlist(strsplit(x, "_"))
...
data_all <- data_all |>
  dplyr::mutate(
    sample = sapply(sample, function(x) {
      parts <- unlist(strsplit(x, "_"))
      if (length(parts) >= 2 && all(grepl("^SPY", parts))) {
...
data_all_meta <- data_all |>
  dplyr::filter(sample %in% metadatas$pool)
##### all samples present
...
data_all <- data_all |>
  dplyr::mutate(
    `Squatina squatina` = ifelse(
      sample %in% ls_conta_shark$sample,
      0,
      `Squatina squatina`
    )
  )
...
species_cols <- dplyr::setdiff(names(data_all_meta), c("sample", "metric"))
...
message(
  length(species_cols) - length(non_empty_species),
  " empty species columns removed."
)
#### zero species removed
...
species_rename_map <- c(
  ...
  "Copt. rendalli_Oreo. niloticus" = "Coptodon_rendalli.Oreochromis_niloticus", # freshwater do not keep in later analysis
...
if (length(existing_rename) > 0) {
  ...
  message(
    "None of the listed species names found in data_all — nothing to rename."
  )
}
...
# Define freshwater complex
freshwater_complex <- "Coptodon_rendalli.Oreochromis_niloticus"


data_all <- data_all |>
  # Remove freshwater complex (safe if absent)
  dplyr::select(-any_of(freshwater_complex)) |>

  # Rename complex to stay at the specific level
  dplyr::rename(
    # Species where geographical distributions help to choose (5 complexes)
    "Cheilopogon_heterurus" = "Cheilopogon_heterurus.Hirundichthys_speculiger", # Hirundichthys speculiger mostly tropical open water; Cheilopogon heterurus Mediterranean (https://fishbase.se/summary/1029 ; https://fishbase.se/summary/Hirundichthys-speculiger) → KEEP Cheilopogon heterurus
    "Trachurus_mediterraneus" = "Trachurus_mediterraneus.Trachurus_trachurus", # T. mediterraneus common Med; T. trachurus mostly Atlantic + vulnerable (https://fishbase.se/summary/trachurus-mediterraneus ; https://fishbase.se/summary/Trachurus-trachurus.html) → KEEP T. mediterraneus
    "Trisopterus_capelanus" = "Trisopterus_capelanus.Trisopterus_minutus", # T. minutus absent Med; T. capelanus present (https://fishbase.se/summary/trisopterus-minutus ; https://fishbase.se/summary/Trisopterus-capelanus.html) → KEEP T. capelanus
    "Notoscopelus_elongatus" = "Notoscopelus_elongatus.Notoscopelus_kroyeri", # N. kroyeri Atlantic endemic; N. elongatus in Med (https://fishbase.se/summary/Notoscopelus-kroyeri ; https://fishbase.se/summary/841) → KEEP N. elongatus
    "Sphyraena_sphyraena" = "Sphyraena_chrysotaenia.Sphyraena_sphyraena", # S. sphyraena common Med; S. chrysotaenia Lessepsian migrant (https://fishbase.se/summary/sphyraena-sphyraena ; https://fishbase.se/Summary/SpeciesSummary.php?id=16905&lang=french) → KEEP S. sphyraena

    # All species found in Med: keep as complex (no renaming needed) :
    "Parablennius_tentacularis.Parablennius_zvonimiri" = "Parablennius_tentacularis.Parablennius_zvonimiri", # both in study area
    "Parablennius_incognitus.Parablennius_sanguinolentus" = "Parablennius_incognitus.Parablennius_sanguinolentus", # both in study area
    "Gaidropsarus_biscayensis.Gaidropsarus_vulgaris" = "Gaidropsarus_biscayensis.Gaidropsarus_vulgaris", # both in study area
    "Chelidonichthys_lucerna.Lepidotrigla_dieuzeidei" = "Chelidonichthys_lucerna.Lepidotrigla_dieuzeidei", # both in study area
    "Chelidonichthys_obscurus.Chelidonichthys_lastoviza" = "Chelidonichthys_obscurus.Chelidonichthys_lastoviza", # both in study area
    "Eutrigla_gurnardus.Trigla_lyra" = "Eutrigla_gurnardus.Trigla_lyra", # both in study area
    "Dentex_dentex.Pagrus_auriga.Pagrus_pagrus" = "Dentex_dentex.Pagrus_auriga.Pagrus_pagrus", # all in study area
    "Raja_asterias.Raja_clavata.Raja_polystigma" = "Raja_asterias.Raja_clavata.Raja_polystigma" # all in study area
  ) |>

  # Complexes where one species is also present alone: merge into complex and drop alone species
  dplyr::mutate(
    "Spicara_flexuosum.Spicara_smaris" = ifelse(
      `Spicara_flexuosum.Spicara_smaris` == 1 | `Spicara smaris` == 1,
      1,
      0
    ),
    "Argyrosomus_regius.Umbrina_cirrosa" = ifelse(
      `Argyrosomus_regius.Umbrina_cirrosa` == 1 | `Umbrina cirrosa` == 1,
      1,
      0
    )
  ) |>
  dplyr::select(-any_of(c("Umbrina cirrosa", "Spicara smaris"))) |>

  # Remove redundant overlapping complex (no extra information)
  dplyr::select(
    -any_of("Labrus_merula.Labrus_viridis.Centrolabrus_melanocercus")
  )
...
existing <- dplyr::intersect(names(species_corrections), names(data_all))
...
Med_fishbase <- readr::read_delim(
  "data/raw_data/Med_fishbase.csv",
  delim = ";",
  escape_double = FALSE,
  trim_ws = TRUE
)
...
Med_fishbase <- Med_fishbase |>
  dplyr::mutate(
    Species = ifelse(
      ...
      "Bathytoshia lata",
      Species
    ), # see : https://www.fishbase.se/summary/Bathytoshia-centroura
    Status = ifelse(Species == "Bathytoshia lata", "native", Status),
    Species = ifelse(
      Species == "Gobius ophiocephalus",
      "Zosterisessor ophiocephalus",
      Species
    ) # Change name of Gobius ophiocephalus with real validated name under WORMS : Zosterisessor ophiocephalus
  )
...
France_fishbase <- readr::read_csv(
  "data/raw_data/France_fishbase.csv"
)
...
out_of_range_species1_2 <- readr::read_csv(
  "data/raw_data/out_of_range_species1.2.csv"
)


# Keep only rows where Decision == "OUT"
out_species <- out_of_range_species1_2 |>
  dplyr::filter(toupper(Decision) == "OUT") |>
  dplyr::pull(Species) |>
  unique()
...
cat("\nCross-check summary:\n")
cat(
  "• Total species not in reference list: ",
  length(species_not_in_ref_clean),
  "\n"
)
cat(
  "• Of these, marked as 'OUT' in local dataset: ",
  length(species_confirmed_out),
  "\n"
)
cat("• Remaining (not flagged as OUT): ", length(species_not_flagged_out), "\n")

if (length(species_confirmed_out) > 0) {
  cat("\n Species confirmed as OUT:\n")
  print(species_confirmed_out)
}

if (length(species_not_flagged_out) > 0) {
  cat("\n Species not in reference but not flagged as OUT:\n")
...
# ───────────────────────────────────────────────────────────────────────────
# ─── Step 5: Export presence data frames ────────────────----
# ───────────────────────────────────────────────────────────────────────────
# Split by metric type

# Number of PCR replicates
data_rep <- data_all_final |>
  dplyr::filter(metric == "nb_rep") |>
  dplyr::select(-metric) # remove metric column

# Number of reads
data_seq <- data_all_final |>
  dplyr::filter(metric == "nb_seq") |>
  dplyr::select(-metric) # remove metric column

# Presence/absence (based on nb_rep)
data_pres <- data_rep |>
  dplyr::mutate(across(-sample, ~ ifelse(. > 0, 1, 0))) # convert detections to 1/0


# Save all 3 datasets to CSV
write.table(
  data_rep,
  "data/raw_data/data_MED_teleo_nb_rep_1824_V1.csv",
  row.names = FALSE,
  dec = ".",
  sep = ";"
)
...
write.table(
  data_seq,
  "data/raw_data/data_MED_teleo_nb_seq_1824_V1.csv",
  row.names = FALSE,
  dec = ".",
  sep = ";"
)
...
write.table(
  data_pres,
  "data/raw_data/data_MED_teleo_pres_1824_V1.csv",
  row.names = FALSE,
  dec = ".",
  sep = ";"
)
